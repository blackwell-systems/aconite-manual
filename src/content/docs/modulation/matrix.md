...
The matrix has **eight slots**. Each row is one connection:
...
A row with Source set to **—** is empty and does nothing. The eight slots are all
...
- **Velocity** and **Keytrack** — per-note performance values
...
| Transform | Effect |
|-----------|--------|
| **—** | No transform; the source passes through unchanged. |
...
From the **grid**: set the Source to **—** (Off) or set Depth to zero.
